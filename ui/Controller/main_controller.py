...
from Window.upload_dialog import UploadDialog
...
class MainController:
    ...
    def on_upload_file(self):
        ...
        if not path:
            return
        self._start_upload(path, self.main_binder.current_folder_id)

    def on_upload_folder(self):
        root_dir = QFileDialog.getExistingDirectory(None, "Chọn thư mục để tải lên")
        if not root_dir:
            return
        self._start_upload(root_dir, self.main_binder.current_folder_id, is_folder=True)
    ...
    def _start_upload(self, path: str, parent_folder_id: int, is_folder: bool = False):
        if not path:
            return
        self._upload_target = (path, parent_folder_id)
        dialog = self._ensure_upload_dialog()
        dialog.start(path, parent_folder_id)
        def handle(result):
            self._handle_upload_finished(result)
        task_fn = (lambda: self.main_binder.upload_folder(path)) if is_folder else (lambda: self.main_binder.upload_file(path))
        self._run_task(
            "Đang tải lên...",
            task_fn,
            handle,
    ...
    def _ensure_upload_dialog(self):
        if self._upload_dialog is None:
            self._upload_dialog = UploadDialog(self._cache_path, self.view)
            self._upload_dialog.cancel_requested.connect(self._on_upload_cancel_requested)
            self._upload_dialog.retry_requested.connect(self._on_upload_retry_requested)
            self._upload_dialog.closed.connect(self._on_upload_dialog_closed)
        return self._upload_dialog
    ...
    def _handle_upload_finished(self, result):
        success, msg = result
        ...
        if self.main_binder.isDisconnected:
            ...
            return
        if dialog:
            dialog.finish(success, msg or ("Tải lên thành công" if success else "Tải lên thất bại"))
        if success:
            QMessageBox.information(self.view, "Tải lên", msg or "Tải lên thành công")
        elif msg:
            QMessageBox.warning(self.view, "Tải lên", msg or "Tải lên thất bại")
        self.reload()
    ...
    def _handle_upload_error(self, exc: Exception):
        dialog = self._upload_dialog
        if dialog:
            dialog.finish(False, str(exc))
        QMessageBox.warning(self.view, "Tải lên", str(exc))
    ...
    def _on_upload_cancel_requested(self, session_id: str):
        if not session_id:
            QMessageBox.warning(self.view, "Hủy tải", "Không tìm thấy phiên upload hiện tại")
            return
        dialog = self._upload_dialog
        def handle(result):
            success, msg = result
            if dialog:
                dialog.show_cancel_result(success, msg or ("Đã gửi yêu cầu hủy" if success else "Hủy upload thất bại"))
            if not success and msg:
                QMessageBox.warning(self.view, "Hủy tải", msg)
        self._run_task(
            "Đang hủy upload...",
            lambda: self.main_binder.cancel_upload_session(session_id),
            handle,
            show_loading=False,
        )
    ...
    def _on_upload_retry_requested(self):
        dialog = self._upload_dialog
        ok = self.main_binder.reconnect()
        if not ok:
            if dialog:
            ...
            return
        if dialog:
            dialog.show_status("Kết nối lại thành công. Đang thử tải lên...", is_error=False)
            if self._upload_target:
                path, parent_folder_id = self._upload_target
                dialog.start(path, parent_folder_id)
        def handle(result):
            self._handle_upload_finished(result)
        self._run_task(
            "Đang tải lên...",
            lambda: self.main_binder.resume_upload_session(),
            handle,
            self._handle_upload_error,
            show_loading=False,
        )
    ...
    def _on_upload_dialog_closed(self):
        self._upload_dialog = None
