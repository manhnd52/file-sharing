...
class UploadDialog(QDialog):
    ...
    def __init__(self, cache_path: str, parent: Optional[QWidget] = None):
        ...
        self._file_path = ""
        self._parent_folder_id = 0

        self._build_ui()

    def _build_ui(self):
        self.setWindowTitle("Đang tải lên")
        self.setModal(True)
        ...
        info_layout.addWidget(self._build_row("Tệp", self.file_value))
        info_layout.addWidget(self._build_row("Tải lên tới", self.dest_value))
        ...
        self.status_label = QLabel("Đang tải lên...")
    ...
    def start(self, file_path: str, parent_folder_id: int):
        self._current_session = ""
        self._file_path = file_path or ""
        self._parent_folder_id = parent_folder_id or 0
        self.file_value.setText(self._file_path or "-")
        self.dest_value.setText(f"Thư mục ID {self._parent_folder_id}" if self._parent_folder_id else "-")
        self.session_value.setText("Session: Đang khởi tạo...")
        self.progress_bar.setMaximum(100)
        self.progress_bar.setValue(0)
        self.progress_text.setText("0% • 0 / 0")
        self.show_status("Đang tải lên...", is_error=False)
        self.retry_btn.hide()
    ...
    def finish(self, success: bool, message: str = ""):
        ...
        self.show_status(message or ("Tải lên thành công" if success else "Tải lên thất bại"), is_error=not success)
    ...
    def show_cancel_result(self, success: bool, message: str = ""):
        self.show_status(message or ("Đã gửi yêu cầu hủy" if success else "Hủy upload thất bại"), is_error=not success)
        if success:
            self.cancel_btn.setEnabled(False)
        else:
            self.cancel_btn.setEnabled(True)
    ...
    def _refresh_from_cache(self):
        ...
        file_path = node.get("file_path") or self._file_path
        if file_path:
            self._file_path = file_path
            self.file_value.setText(file_path)

        parent_folder_id = int(node.get("parent_folder_id") or self._parent_folder_id)
        if parent_folder_id:
            self._parent_folder_id = parent_folder_id
            self.dest_value.setText(f"Thư mục ID {parent_folder_id}")

        total_size = self._to_int(node.get("total_size"))
        chunk_size = self._to_int(node.get("chunk_size"))
        last_chunk = self._to_int(node.get("last_sent_chunk"))
        sent = max(0, last_chunk) * max(0, chunk_size)

        if total_size > 0:
            percent = min(100, int(sent * 100 / total_size))
            self.progress_bar.setMaximum(100)
            self.progress_bar.setValue(percent)
        else:
            percent = 0
            self.progress_bar.setMaximum(0)
        self.progress_text.setText(f"{percent}% • {self._format_size(sent)} / {self._format_size(total_size)}")
